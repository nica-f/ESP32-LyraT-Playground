...
bool ES8388::setOutputVolume(uint8_t vol) {
  ...
  if (_outSel == OUT1) {
    ...
    res &= write_reg(ES8388_DACCONTROL26, 0);  // LOUT2VOL
    res &= write_reg(ES8388_DACCONTROL27, 0);  // ROUT2VOL
  } else if (_outSel == OUT2) {
    res &= write_reg(ES8388_DACCONTROL24, 0);  // LOUT1VOL
    res &= write_reg(ES8388_DACCONTROL25, 0);  // ROUT1VOL
